...
fn tables_for_session_fn(session_id: &SessionId) -> impl Fn(&(&TableSelector, &InMemoryTable)) -> bool + '_ {
    move |(k, _): &(_, _)| {
...
impl InMemoryStore {
    ...
    fn get_tables_for_session(&self, session_id: &SessionId) -> Vec<(TableSelector, InMemoryTable)> {
        self.tables.lock().unwrap()
            .iter()
