    ...
    loop {
        {
            let mut changes = vec![];
            {
                ...
                let apply_nums =
                    |s: &mut Vec<Action>,
                     net: &IpNet,
                     x: &Vec<(u32, Arc<CompressedRouteAttrs>)>,
                     y: &Vec<(u32, Arc<CompressedRouteAttrs>)>| {
                        diff(
                            s,
                            x,
                            y,
                ...
                diff(
                    ...
                    |s, x, y| {
                        apply_nums(
                            s,
                            x,
                            table.table.exact(&x).unwrap(),
                            rib_out.table.exact(&y).unwrap(),
                        );
                    },
                );
            }
            ...
            for change in changes {
                match change {
                    Action::Update(net, num, attrs) => {
                        rib_out.update_route_compressed(num, net, attrs).await
                    }
